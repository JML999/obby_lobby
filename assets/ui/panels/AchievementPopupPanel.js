    ...
    setupEventListeners() {
        if (window.hytopia && window.hytopia.onData) {
            window.hytopia.onData((data) => {
                console.log('[AchievementPopupPanel] Received data:', data);
                if (data.type === 'achievementPopup') {
                    console.log('[AchievementPopupPanel] Processing achievement popup:', data);
                    this.addMessage(data);
                }
            });
            console.log('[AchievementPopupPanel] Event listeners set up successfully');
        } else {
            console.warn('[AchievementPopupPanel] hytopia not available, event listeners not set up');
        }
    }
    
    addMessage(data) {
        console.log('[AchievementPopupPanel] Adding message:', data);
        const message = {
